(ns sakkyokuka.commission
  "受注（commission）— 作曲家の商売の単位。

  `ongakuka` の単位が『カタログの1曲を選んで使わせる』であるのに対し、
  こちらの単位は『依頼を受けて作り、権利を定めて渡す』。受注1件は
  作品・納品物・譲渡する権利・対価を1つに束ね、成立するかどうかを
  `validate` が判定する。

  ここが商売層の gate — 受注を受ける前に、その約束が
  (a) 持っていない権利を売っていないか（`sakkyokuka.rights`）
  (b) 作れない物を約束していないか（`sakkyokuka.work`）
  を構造的に確かめる。"
  (:require [clojure.string :as str]
            [sakkyokuka.rights :as rights]
            [sakkyokuka.work :as work]))
...
(defn commission
  ...
  `:commission/grants` は依頼主へ譲渡する権利（`sakkyokuka.rights/right`）、
  `:commission/deliverables` は納品する種別のベクタ
  （`sakkyokuka.work/deliverable-kinds` のキー）。"
