(ns sakkyokuka.rights
  "原盤権・著作権・出版権の保有と譲渡のモデル、および譲渡の構造的拒否。

  この namespace が存在する理由は一つ: **持っていない権利は売れない**。
  `ongakuka` が扱う既製カタログ資産（DOVA-SYNDROME / Incompetech 等）に
  ついて studio が持つのは使用許諾だけで原盤権ではないから、その音源を
  素材にした納品で `:master` を exclusive 譲渡することは商売として
  成立しない。`kotoba-lang/ongaku` が license policy 違反の利用を構造的に
  拒否するのと同じことを、権利の譲渡側で行う。
